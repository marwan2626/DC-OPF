...
def solve_opf3(net, time_steps, const_load_heatpump, const_load_household, Bbus):
    ...
    for t in time_steps:
        ...
        for bus in net.load.bus.values:
            # Store updated load values in the temporary dictionary
        ...
        flexible_load_vars[t] = model.addVars(flexible_load_buses, lb=0,
                                          ub=net.load.loc[net.load['controllable'] == True, 'p_mw'].values,
                                          name=f'flexible_load_{t}')
        flex_load_curtailment_vars[t] = model.addVars(flexible_load_buses, lb=0,
                                                  ub=net.load.loc[net.load['controllable'] == True, 'p_mw'].values,
                                                  name=f'flex_load_curtailment_{t}')

        for bus in flexible_load_buses:
            original_load = net.load.loc[net.load.bus == bus, 'p_mw'].values[0]
    ...
    if model.status == gp.GRB.OPTIMAL:
        ...
        for t in time_steps:
            ...
            load_results[t] = {
                ...
                net.load.loc[net.load.bus == bus, 'p_mw'].values[0] if not net.load.loc[net.load.bus == bus, 'p_mw'].empty else 0
                for bus in net.bus.index
            }
